...
local function execute(strategy, statement_name, attributes, options)
  ...
  local update
  local ttl_enabled = strategy.schema.ttl
  local ttl
  if options then
  ...
  for i = 1, argc do
    local name  = argn[i]
    local value
    if name == "ttl" and ttl_enabled then
      if ttl then
        if ttl == 0 then
          argv[i] = escape_literal(connector, null, fields.ttl)
        elseif not update and type(attributes.created_at) == "number" then
          argv[i] = escape_literal(connector, ttl + attributes.created_at, fields.ttl)
        elseif update and type(attributes.updated_at) == "number" then
          argv[i] = escape_literal(connector, ttl + attributes.updated_at, fields.ttl)
        elseif ttl then
          argv[i] = escape_literal(connector, ttl + time(), fields.ttl)
        end
...
function _mt:insert(entity, options)
  local res, err
  if self.schema.ttl and options and options.ttl then
    res, err = execute(self, "insert", self.collapse(entity), options)
  else
    res, err = execute(self, "insert", self.collapse(entity))
  end
