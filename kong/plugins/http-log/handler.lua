...
local LuaSerializer = require "kong.plugins.log-buffering.lua_serializer"
local JSONSerializer = require "kong.plugins.log-buffering.json_serializer"
...
function HttpLogHandler:log(conf)
  ...
  if not buf then
    ...
    if conf.queue_size > 1 then
      ...
      -- Keep it as a 1-entry Lua array.
      -- Will produce a backward-compatible single-object HTTP request.
      buffer_serializer = LuaSerializer.new()
    end

    local err
    buf, err = Buffer.new(self.name, conf, buffer_serializer, Sender.new(conf, self.ngx_log), self.ngx_log)
    if not buf then
