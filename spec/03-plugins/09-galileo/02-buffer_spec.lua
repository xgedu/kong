...
local function reload_buffer()
  ...
  Serializer = require "kong.plugins.galileo.serializer"
...
describe("ALF Buffer", function()
  ...
  it("sanity", function()
    local buf = assert(Buffer.new("test", conf, Serializer.new(conf), Sender.new(conf), log))
    assert.equal(0, #buf.sending_queue)
    assert.equal(0, #buf.serializer.cur_alf.entries)
  ...
  it("sane defaults", function()
    local buf = assert(Buffer.new("test", {
      ...
      host = "",
      port = 80
    }, Serializer.new(conf), Sender.new(conf), log))
    ...
    assert.False(buf.serializer.cur_alf.log_bodies)
  end)
  it("returns error on invalid conf", function()
    local buf, err = Buffer.new()
    assert.equal("arg #1 (id) must be a string", err)
    assert.is_nil(buf)

    buf, err = Buffer.new("test")
    assert.equal("arg #2 (conf) must be a table", err)
    assert.is_nil(buf)

    buf, err = Buffer.new("test", {})
    assert.equal("arg #3 (serializer) must be a table", err)
    assert.is_nil(buf)

    buf, err = Buffer.new("test", {}, {})
    assert.equal("arg #3 (serializer) must include an add_entry function", err)
    assert.is_nil(buf)

    buf, err = Buffer.new("test", {}, Serializer.new(conf))
    assert.equal("arg #4 (sender) must be a table", err)
    assert.is_nil(buf)

    local s
    s, err = Serializer.new({server_addr = "10.10.10.10"})
  ...
  describe("add_entry()", function()
    it("adds an entry to the underlying ALF serializer", function()
      ...
      for i = 1, 10 do
        ...
        assert.equal(i, #buf.serializer.cur_alf.entries)
      end
    end)
    it("calls flush() if the number of entries reaches 'queue_size'", function()
      ...
      assert.equal(conf.queue_size - 1, #buf.serializer.cur_alf.entries)
      assert(buf:add_entry(_ngx, nil, nil))
  ...
  describe("flush()", function()
    it("JSON encode the current ALF and add it to sending_queue", function()
      ...
      assert.equal(0, #buf.serializer.cur_alf.entries) -- flushed
